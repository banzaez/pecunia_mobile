...
class TransactionController extends BaseController {
  ...
  Future<void> addSQL(Transaction transaction) async {
    ...
    await _sqlController.tableTransactions.add(value: transaction);
  ...
  Future<void> updateSQL(Transaction transaction) async {
    transaction.id = _walletId.value;
    await _sqlController.tableTransactions.update(value: transaction);
    await refreshTransactions();
    notifyListenersSQL("add");
  }

  Future<void> deleteSQL(int id) async {
    await _sqlController.tableTransactions.delete(id: id);
    await refreshTransactions();
    notifyListenersSQL("add");
  }

  Future<void> refreshTransactions() async => transactions.value =
      await _sqlController.tableTransactions.selectAllByWalletId(_walletId.value);
}
